...
bases.fromAlphabet = function (str, alphabet) {
    ...
    while (str.length) {
        ...
        num = num.plus(Big(base).pow(pos).times(alphabet.indexOf(c)));
        pos++;
    }
...
function delnumber(number, decimal) {
	number = number + '';
	...
	number = bases.fromBase(number, 10);
	number = bases.toBase(number, 16);
	console.log(number);
	return number;
}

function delnumber10(number, decimal) {
	number = number + '';
	number = scientificNotationToString(number);
	//小数点位置
	let a = number.indexOf('.');
